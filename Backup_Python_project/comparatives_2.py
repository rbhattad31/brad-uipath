...
def main():
    ...
    with pd.ExcelWriter("Output.xlsx", engine='xlsxwriter', engine_kwargs={'options': {'strings_to_numbers': True}}, mode='w') as writer:
        ...
        month_concentration_pd.to_excel(writer, sheet_name="Concentrations", index=False, startrow=purchase_pd_rows + 2, startcol=0)
        plant_concentration_pd.to_excel(writer, sheet_name="Concentrations", index=False, startrow=purchase_pd_rows + month_pd_rows + 4, startcol=0)
        dom_import_concentration_pd.to_excel(writer, sheet_name="Concentrations", index=False, startrow=purchase_pd_rows + month_pd_rows + plant_pd_rows + 6, startcol=0)
