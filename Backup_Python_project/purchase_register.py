...
def generate_purchase_type_wise(excel_file_pd, previous_quarter_final_file):
    ...
    previous_quarter_final_file_pd = pd.read_excel(previous_quarter_final_file,
                                                   sheet_name="Purchase Type Wise Comparatives", usecols="A,B,D")
    ...
    purchase_type_wise_comparatives_pd = pd.merge(purchase_type_wise_pd, previous_quarter_final_file_pd, how="outer",
    ...
    purchase_type_wise_comparatives_pd.drop(purchase_type_wise_comparatives_pd.index[
                                                (purchase_type_wise_comparatives_pd[columns_names[2]] == 0) & (
                                                            purchase_type_wise_comparatives_pd[columns_names[3]] == 0)],
                                            inplace=True)
...
def generate_month_wise(excel_file_pd, previous_quarter_final_file):
    ...
    month_wise_pd = pd.pivot_table(excel_file_pd, index=["Month"], values="GR Amt.in loc.cur.", aggfunc=numpy.sum,
    ...
    previous_quarter_final_file_pd = pd.read_excel(previous_quarter_final_file, sheet_name="Month Wise Comparatives",
                                                   usecols="C,D")
...
def generate_plant_type_wise(excel_file_pd, previous_quarter_final_file):
    # create pivot table
    plant_type_wise_pd = pd.pivot_table(excel_file_pd, index=["Plant"], values="GR Amt.in loc.cur.", aggfunc=numpy.sum,
    ...
    previous_quarter_final_file_pd = pd.read_excel(previous_quarter_final_file, sheet_name="Plant Wise Comparatives",
                                                   usecols="A,C")

    # merging present and previous quarter purchase type wise data
    plant_type_wise_comparatives_pd = pd.merge(plant_type_wise_pd, previous_quarter_final_file_pd, how="outer",
                                               on=["Plant"])
...
def generate_domestic_and_import_wise(excel_file_pd, previous_quarter_final_file):
    ...
    domestic_and_import_wise_pd = pd.pivot_table(excel_file_pd, index=["Purchase Type"], values="GR Amt.in loc.cur.",
    ...
    # read previous quarters final working file
    previous_quarter_final_file_pd = pd.read_excel(previous_quarter_final_file, sheet_name="Dom&Imp Wise Comparatives",
                                                   usecols="A,C")

    # merging present and previous quarter purchase type wise data
    domestic_and_import_wise_comparatives_pd = pd.merge(domestic_and_import_wise_pd, previous_quarter_final_file_pd,
...
def financial_quarter_title_update(file, quarter):
    ...
    worksheet_month_wise.cell(1,
                              3).value = "Month"  # C1 cell - Its value is changed to Month.1 while processing, adjusting it back to "Month"
...
def concentration_formatting(file, purchase_pd_rows, month_pd_rows, plant_pd_rows, dom_import_pd_rows,
                             financial_quarter):
    workbook = openpyxl.load_workbook(file)
    fill_color = '00a2ed'
    ...
    for i in range(1, max_row + 1):
        # first rows code
        if i == purchase_start_row or i == month_start_row or i == plant_start_row or i == dom_import_start_row:
        ...
        # formatting each concentration last row 
        elif i == purchase_end_row or i == month_end_row or i == plant_end_row or i == dom_import_end_row:
            for j in range(1, max_column + 1):
                ...
                if i == purchase_end_row:
                    if j == 3:
                        ...
                        pass
                elif i == month_end_row or i == plant_end_row or i == dom_import_end_row:
                    if j == 2:
                        concentration_sheet.cell(i, j).number_format = '#,###.##'
                    elif j == 3:
                        concentration_sheet.cell(i, j).number_format = '##.##%'
...
def main(input_file_path, previous_file_path, output_file_path):
    ...
    purchase_pd_rows = len(
        purchase_concentration_pd.index) + 1  # this len() method not including heading row in counting, so adding 1 to get total rows
    ...
    with pd.ExcelWriter(output_file_path, engine='xlsxwriter', engine_kwargs={'options': {'strings_to_numbers': True}},
        ...
        month_concentration_pd.to_excel(writer, sheet_name="Concentrations", index=False, startrow=purchase_pd_rows + 1,
                                        startcol=0)
        plant_concentration_pd.to_excel(writer, sheet_name="Concentrations", index=False,
                                        startrow=purchase_pd_rows + month_pd_rows + 2, startcol=0)
        dom_import_concentration_pd.to_excel(writer, sheet_name="Concentrations", index=False,
                                             startrow=purchase_pd_rows + month_pd_rows + plant_pd_rows + 3, startcol=0)
