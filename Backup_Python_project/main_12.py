...
def generate_purchase_type_wise(excel_file_pd):
    ...
    # dropping columns present and previous quarters both have values as zero - Improvement needed
    purchase_type_wise_comparatives_pd.drop(purchase_type_wise_comparatives_pd.index[(purchase_type_wise_comparatives_pd['GR Amt.in loc.cur.'] == 0) & (purchase_type_wise_comparatives_pd['Q3 FY 21-22'] == 0)], inplace=True)
    # purchase_type_wise_comparatives_pd.drop(purchase_type_wise_comparatives_pd.index[(purchase_type_wise_comparatives_pd.iloc[[], 2] == 0) & (purchase_type_wise_comparatives_pd.iloc[[], 3] == 0)],inplace=True)
...
def find_financial_year_and_quarter(excel_file_pd):
    ...
    months = excel_file_pd['Month'].unique().tolist()
    print(months)       # output example ['Jan', 'Feb', 'Mar']

    years = excel_file_pd['Year'].unique().tolist()
    print(years)        # output example [2022]
